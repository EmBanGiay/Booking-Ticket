...
function App() {
  ...
  /*
  useEffect(() => {
    console.log("Use Effect is executed only once!");
    ...
    Initialize(function (tokenData) {
      ...
      //Lấy Record của Event từ Creator
      GetCurrentEvent(eventId, function (creatorEventData) {
        try {
          creatorEvent = creatorEventData["data"];
          ...
          GetPayon(creatorEvent["Payon"]["ID"]).then(function (payonData) {
            if (debugging) {
              console.log("Payon: ");
              console.log(payonData);
            }
          ...
          console.log(e);
          alert("Đã xảy ra lỗi, vui lòng kiểm tra lại đường dẫn");
          document.getElementById('form-1').innerHTML = '';
        }
